...
import com.example.bonds_backend.repository.CounterPartyRepository;
...
public class CounterPartyController {
    ...
    @GetMapping("/getCounterPartyById/{id}")
    @CrossOrigin(origins = "*")

    public ResponseEntity<Object> getById(@PathVariable long id){
        Optional<Counterparty> counterPartyDetails = counterPartyRepo.findById(id);

        if(counterPartyDetails.isEmpty()){
           return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Security not found");
        }

        return ResponseEntity.ok(counterPartyDetails.get());
    }
